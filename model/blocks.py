...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
	...
	"""
	def forward(self, x, emb=None, cond=None, lateral=None, index_mask=None):
		for layer in self:
			if isinstance(layer, TimestepBlock):
				x = layer(x, emb=emb, cond=cond, lateral=lateral, index_mask=index_mask)
			else:
				x = layer(x)
		return x
...
class ResBlock(TimestepBlock):
	...
	def __init__(self, conf: ResBlockConfig):
		...
		if conf.use_condition:
			...
			if conf.two_cond:
				...
				self.cond_emb_layers_z = nn.Sequential(
					nn.SiLU(),
					linear(int(conf.cond_emb_channels / 2), conf.out_channels),
				)
				self.cond_emb_layers_s = nn.Sequential(
					nn.SiLU(),
					linear(int(conf.cond_emb_channels / 2), conf.out_channels),
				)
				# self.cond_emb_layers_z = nn.Sequential(
				# 	nn.SiLU(),
				# 	linear(int(conf.cond_emb_channels), conf.out_channels),
				# )
				# self.cond_emb_layers_s = nn.Sequential(
				# 	nn.SiLU(),
				# 	linear(int(conf.cond_emb_channels), conf.out_channels),
	...
	def forward(self, x, emb=None, cond=None, lateral=None, index_mask=None):
		...
		"""
		return torch_checkpoint(self._forward, (x, emb, cond, lateral, index_mask),
		...
		if self.conf.use_condition:
			...
			if self.conf.two_cond:
				...
				if cond is None:
					...
					cond_out_z = self.cond_emb_layers_z(cond[:, :int(self.conf.cond_emb_channels / 2)]).type(h.dtype)
					cond_out_s = self.cond_emb_layers_s(cond[:, int(self.conf.cond_emb_channels / 2):]).type(h.dtype)
					# cond_out_z = self.cond_emb_layers_z(cond).type(h.dtype)
					# cond_out_s = self.cond_emb_layers_s(cond).type(h.dtype)
					
					cond_out = th.cat([cond_out_z, cond_out_s], dim=1)
...
def apply_conditions(
	...
	"""
	# print('cond in apply_conditions : ')
	# print(cond)
	...
	for i, (scale, shift) in enumerate(scale_shifts):
		# print('i = {}, scale = {}, shift = {}'.format(i, scale.size(), shift.size()))
		# print('biases = {}'.format(biases[i]))
		# i = 0 -> time emb, i = 1 -> cond emb
		# only mask_threshold when training
		# if i == 1:
		# 	# assert index_mask is not None
		# 	if index_mask is not None:
		# 	# index_mask = np.where(np.random.rand(cond.size(0)) <= 0.1)
		# 		scale[index_mask] = 0.
		# 	# scale = th.zeros_like(scale)
