...
class GaussianDiffusionBeatGans:
	...
	def training_losses(self,
		...
		if model_kwargs is None:
			model_kwargs = {}
		if noise is None:
			# 1. 准备噪声：从标准正态分布中采样一个与输入图像x_start相同形状的噪声
			noise = th.randn_like(x_start)

		# 2. 前向加噪：调用q_sample函数，将x_start和t作为输入，生成加噪后的图像x_t
		x_t = self.q_sample(x_start, t, noise=noise)

		# 初始化一个字典，用于存储所有计算出的损失项和中间结果
		terms = {'x_t': x_t}
		cls_loss = 0.
		kld_loss = 0.
		mode = 'eval'
		...
		if self.loss_type in [
			...
			with autocast(self.conf.fp16):
				...
				if cond is None and is_latent == 0:
					...
					# 3. 因果编码过程（处理清晰图像x_start）
					# 调用模型（BeatGANsAutoencModel）的encode方法处理清晰图像x_start
					# 内部会使用d_LaCIM编码器f_(s,z)(x;theta)
################### 编码过程：从x_start中提取cond（包含z和s）
					tmp = model.encode(x_start, r)
					# 通过重参数化技巧采样得到潜在向量 cond=[z,s]
					cond = tmp['cond']
					
					# 4. 计算分类损失 L_CE(s,y;theta)
					# 从cond中分离出因果特征s（后半部分），并送入分类器dec_y
					# 这是为了确保s中包含足够的类别判别信息
################### 使用核心特征s预测标签y
...
################### KL散度损失（用于特征分离）
					...
					# 5. 计算KL散度损失 D_KL(p(s,z|x)||q(s,z))
					# 这是VAE的正则化项，用于约束潜在空间的结构
...
############### 计算分类损失
				if target_y is not None and is_latent == 0:
					...
					# 使用步骤4中得到的预测pred_y和真实标签target_y计算交叉熵损失
					# 对应论文公式(7)中的 γ * L_CE(s,y;θ) 项
			...
			if is_latent == 0:
				...
				if mode == 'train':
					if self.conf.use_club:
						# 从cond中分离出z和s
						z = cond[:, :model.lacim.z_dim]  # 非核心特征z
						s = cond[:, model.lacim.s_dim:]  # 核心特征s
						for _ in range(5):
